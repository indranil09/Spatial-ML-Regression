## ---------------------------------------------------------------------------
## Cross-platform parallel wrapper: fork on macOS/Linux, serial on Windows
## (parallel::mclapply's mc.cores>1 is not supported on Windows).
## ---------------------------------------------------------------------------
safe_mclapply <- function(X, FUN, ..., mc.cores = 1L) {
...
## ---------------------------------------------------------------------------
## Force the fields:: versions of rdist() and Matern(). The standalone 'rdist'
## package (pulled in by some spatial dependencies) MASKS fields::rdist and has
## an incompatible signature -- rdist(X, metric = ...) -- which causes:
##   * Error in match.arg(metric, available_metrics)  (2-arg rdist calls)
##   * "longer object length is not a multiple of shorter" warnings (wrong shape)
## Binding them here (in the sourcing environment) takes precedence over the
## attached packages, so these functions always use the intended versions.
## ---------------------------------------------------------------------------
rdist  <- fields::rdist
...
transform_to_ind <- function(formula,
  ...
  indData <- safe_mclapply(1:nrow(Xtrain), FUN=function(idx){
    if(idx==1){
      y <- ytrain[idx]
      w <- 1
      X <- Xtrain[idx,] / sqrt(w)
  ...
  }, mc.cores=ncores) # End safe_mclapply()
  
  ## Apply decorrelating transform to test data
  Xtest <- model.matrix(~ ., data=testData)
  indTestData <- safe_mclapply(1:nrow(Xtest), FUN=function(idx){
...
fit.NN.Matern <- function(formula,locs,nu,gridsize=15,NearNeighs,
  ...
  ll.list <- safe_mclapply(aMw.list, getLL, mc.cores=num.cores)
